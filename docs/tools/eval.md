...
- JS supports the preferred `await agent(prompt, { agent?, label?, model?, schema?, schemaMode?, isolated?, apply?, merge?, handle? })`; legacy positional slots are still implemented.
- Python/Ruby/Julia use keyword arguments (`schema_mode` outside JS), including `model` as one selector or an ordered list.
- `model` overrides the selected agent's frontmatter/settings model for this spawn. Selectors are resolved in order within the active path-scoped `enabledModels` policy; an explicit selector that cannot resolve fails before child resources are allocated. With `handle=true`, auth-aware fallback to the parent model is reported as `model` plus `modelFallback` (`model_fallback` in Python/Ruby/Julia).
- `agent` defaults from the current spawn policy. `schema` overrides agent/session schemas; `schemaMode`/`schema_mode` chooses `permissive` or `strict`.
...
- `handle=true` returns `{ text, output, handle, id, agent }`, optional `model`/fallback provenance, parsed `data`, and isolation metadata instead of only output/data.
