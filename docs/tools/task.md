...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, task, model?, effort?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.mode` is not `none` **and plan mode is disabled**; `effort` exists only when `task.enableEffort=true` (default off).

- **Batch shape** (`task.batch` on): `{ context, tasks: item[] }` — one subagent per item, all run under the same fan-out rules; there is no top-level agent or model field. `context` is **required** shared background rendered into every spawned subagent's system prompt (`CONTEXT` section); `agent`, `model`, `outputSchema`, and `schemaMode` are per item. `effort` is added only when its setting enables it; `isolated` additionally requires plan mode to be disabled.
...
| `model` | `string \\| string[]` | No | Per-spawn model selector or ordered selector list. Takes precedence over settings and agent defaults; role aliases expand through `modelRoles`, candidates stay within the active `enabledModels` scope, and an auth-unusable candidate may fall back to the parent's active model. Item field in batch shape, top-level in flat shape. |
...
7. Model priority: per-spawn `model` → `task.agentModelOverrides` → agent frontmatter → configured task role/session fallback. Role aliases expand through `modelRoles`; ordered candidates resolve within the active path-scoped `enabledModels` policy after model discovery settles. An explicit selector that cannot resolve fails before resources are allocated; an auth-unusable selection reports `resolvedModelIsFallback` in progress/results.
