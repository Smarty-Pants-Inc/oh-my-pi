...
function prepareProviderRequestOptions<T extends Pick<StreamOptions, "fetch" | "providerDispatchGuard">>(
	...
	const fetch = wrapFetchForProxy(withInferenceUserAgent(debugOptions.fetch ?? guardedFetch), model.provider);
...
function withInferenceSessionId(options?: SimpleStreamOptions): SimpleStreamOptions {
	...
	return { ...options, sessionId: crypto.randomUUID(), sessionIdGenerated: true };
}
...
function mapOptionsForApi<TApi extends Api>(
	...
	const base = {
		...
		sessionIdGenerated: options?.sessionIdGenerated,
