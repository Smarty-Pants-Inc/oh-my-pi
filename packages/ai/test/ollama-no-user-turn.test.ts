...
describe("Ollama no-user-turn handling", () => {
	it("does not demote the trailing agent developer turn to user", async () => {
	...
	it("emits typed internal context as system authority and never a user turn", async () => {
		let payload: OllamaChatRequestPayload | undefined;
		const fetchMock = async (_input: string | URL | Request, init?: RequestInit): Promise<Response> => {
			const parsed: unknown = JSON.parse(String(init?.body));
			if (!isOllamaChatRequestPayload(parsed)) throw new Error("Expected Ollama payload object");
			payload = parsed;
			return new Response('{"message":{"content":"ok"},"done":true,"done_reason":"stop"}\n', { status: 200 });
		};
