...
function emitProgress(
	...
	emittedProgress: Map<AsyncJob, Record<string, unknown>>,
): void {
	...
	if (!isUnknownRecord(first) || emittedProgress.get(resolved.job) === first) return;
	emittedProgress.set(resolved.job, first);
...
export async function runEvalWait(
	...
	return await withBridgeTimeoutPause(
		options.emitStatus,
		async () => {
			const emittedProgress = new Map<AsyncJob, Record<string, unknown>>();
			for (const handle of resolved) emitProgress(handle, options.emitStatus, emittedProgress);
