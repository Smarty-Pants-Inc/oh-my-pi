...
export class StreamingEditGuard {
	...
	// Serializes per-file verifications so interleaved deltas do not scan the same
	// target concurrently.
	#verificationChain = new Map<string, Promise<void>>();
	// Queued checks from before an edit result must not validate against the new file.
	#fileEpoch = new Map<string, number>();
	#lastToolCallId: string | undefined;
	// Unlike promptGeneration, this advances at every turn boundary. A check queued
	// before reset() must not start under the next turn.
	#epoch = 0;
	...
	maybeAbort(event: AgentEvent): void {
		if (!this.#host.settings.get("edit.streamingAbort") || this.#abortTriggered) return;
		...
		if (event.type === "tool_stream_update") {
			...
			const failed = (update.files as unknown[]).find(
		...
		const { toolCall, path, resolvedPath, diff, op } = streamingEdit;
		...
		const normalizedDiff = this.#normalizeDiff(diffForCheck);
		...
		if (removedLines.length > 0) this.#queueRemovedLinesCheck(toolCall.id, path, resolvedPath, removedLines);
	}
	...
	#normalizeDiff(diff: string): string {
		const lines = normalizeToLF(diff).split("\n");
		while (lines.length > 0 && lines.at(-1)?.trim() === "") lines.pop();
		if (lines[0]?.trim().match(/^```(?:diff|patch)?$/i)) lines.shift();
		if (lines.at(-1)?.trim() === "```") lines.pop();
		return lines
			.filter(line => {
				if (line.startsWith("+") || line.startsWith("-") || line.startsWith(" ") || line.startsWith("@@"))
					return true;
				return !/^(?:diff --git|index |--- |\+\+\+ |old mode |new mode |similarity index |rename from |rename to )/.test(
					line.trim(),
				);
			})
			.join("\n");
	...
	#resolveSessionFsPath(filePath: string): string | undefined {
		...
		if (normalized.startsWith("local:")) return resolveLocalUrlToPath(normalized, this.#host.localProtocolOptions());
