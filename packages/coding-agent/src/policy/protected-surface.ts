...
const PATH_RULES: readonly PathRule[] = [
	...
	{
		pattern: /(?:^|\/)packages\/catalog\/src\/(?:discovery\/(?:cursor-gen|devin-gen)|wire)\//i,
		surface: "provider-wrapper",
	},
	...
	{
		pattern:
			/(?:^|\/)(?:session\/agent-session|modes\/interactive-mode|session\/(?:async-job-delivery|compaction-methods|irc-bridge|launch-completion|prewalk|session-maintenance|stream-guards|ttsr-coordinator|turn-recovery|unexpected-stop-classifier|yield-queue))\.[cm]?[jt]s$/i,
		surface: "automatic-turn",
	},
	{
	...
	{
		pattern:
			/(?:^|\/)(?:packages\/coding-agent\/src\/internal-urls\/skill-protocol|packages\/coding-agent\/src\/session\/skill-title-input)\.[cm]?[jt]s$/i,
		surface: "skill",
	},
