...
export interface ReadToolDetails {
	...
	isDirectory?: boolean;
	/** False when rendered lines do not map directly to source lines. */
	sourceLineAligned?: boolean;
	/** True when a selector-shaped suffix was part of the literal filename. */
	literalPath?: boolean;
	resolvedPath?: string;
	...
	/** Number of unresolved git conflicts surfaced by this read. */
	conflictCount?: number;
	/** Paths recovered from a delimited read argument. */
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	async #executeInner(
			...
			if (rendered) {
				return markLiteralResult(
					buildInMemorySelectorResult(this.session, rendered, parsed, {
						details: confirmedRegularFileDetails({ resolvedPath: absolutePath, sourceLineAligned: false }),
						sourcePath: absolutePath,
		...
		if (isVideoPath(absolutePath)) {
			return markLiteralResult(
				await this.#readVideoFile(absolutePath, localTarget.sel, fileSize, suffixResolution, signal),
			);
		}
		if (parsed.kind === "image") {
			...
			if (result.ok) {
				...
				details.sourceLineAligned = false;
				sourcePath = absolutePath;
				content = [{ type: "text", text: `[Cannot read ${ext} file: ${result.error}]` }];
			} else {
				details.resolvedPath = absolutePath;
				details.sourceLineAligned = false;
				sourcePath = absolutePath;
			...
			if (looksBinary) {
				return markLiteralResult(
					toolResult<ReadToolDetails>({
						...confirmedRegularFileDetails({ resolvedPath: absolutePath, sourceLineAligned: false }),
						suffixResolution,
					})
						.text(
							prependSuffixResolutionNotice(
		...
		return markLiteralResult(resultBuilder.done());
	}
