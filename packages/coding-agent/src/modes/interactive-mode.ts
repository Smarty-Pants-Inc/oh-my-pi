...
export class InteractiveMode implements InteractiveModeContext {
	...
	/** Session that owns the plan currently rendered in {@link todoPhases}. */
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		this.#signalTeardown = createSessionTeardown({
			...
			disposeSession: async reason => {
				await this.herdrCollabHostLifecycle?.stop("session stopped");
				this.herdrCollabHostLifecycle = undefined;
				this.herdrCollabHost = undefined;
	...
	#scheduleLoopAutoSubmit(): void {
		...
		this.session.recordAutomaticTurnOutcome(
			...
			"rejected",
			"loop mode cannot manufacture direct user input",
		);
	}
	...
	/** Reconcile completed nested work only into a focused worker's visible plan. */
	#reconcileTodosWithSubagents(): void {
		const owner = this.#todoPhasesOwner ?? this.session;
		// The main session owns its canonical todo transitions. This UI helper is
		// only for a focused worker whose nested child has completed.
		if (owner === this.session) return;
	...
	#scheduleObserverUiSync(kind: SessionObserverChangeKind): void {
		if (kind === "lifecycle") this.#observerUiSyncNeedsTodoReconcile = true;
	...
	async handleResumeSession(sessionPath: string): Promise<boolean> {
		...
		return this.#selectorController.handleResumeSession(sessionPath, { settingsFlushed: true });
	}
