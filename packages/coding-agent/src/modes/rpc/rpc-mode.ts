...
export async function runRpcSkillCommand(
	...
	options?: { queueChipText?: string; toolChoice?: ToolChoice },
): Promise<boolean> {
	const built = prebuilt ?? (await buildSkillPromptMessage(invocation.skill, invocation.args, "user"));
	return await session.promptCustomMessage(
		{
		...
		{ streamingBehavior, queueChipText: options?.queueChipText, toolChoice: options?.toolChoice },
	);
}
...
export async function dispatchRpcSkillPrompt(input: {
	...
	watchAndReportLocalOnlyPromptResult({
		id: input.id,
		startPrompt: () =>
			runRpcSkillCommand(input.session, invocation, input.streamingBehavior ?? "steer", built, {
				queueChipText: input.message,
...
export async function tryRunRpcSkillCommand(
	...
	await runRpcSkillCommand(session, invocation, streamingBehavior, undefined, { queueChipText: text, toolChoice });
	return { agentInvoked: true };
}
