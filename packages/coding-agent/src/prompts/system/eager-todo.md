...
NEVER call `{{toolRefs.todo}}` again unless task state or scope materially changed — for example, an item finished, became blocked, was added, or is no longer required.
{{else}}
...
If you create the list, continue the request in the same turn. Reconcile it whenever task state or scope materially changes; avoid redundant calls when nothing changed.
{{/if}}
</system-reminder>
