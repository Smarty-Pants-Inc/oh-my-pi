...
A dequeue revokes only its exact `(pull request, head SHA, request ID)` attempt. Under the active `/smarty-land` authority, a fresh head may queue after fresh local proof, CLEAN review, queue eligibility preflight, and exact-head checks pass. Retry an unchanged head only once for a classified external or transient failure, with unchanged source, still-valid local proof, and a helper-derived fresh request ID. Never retry an unknown, policy, authentication, security, safety, owner-decision, or repeated same-cause failure. After two evidence-valid failures of the same route, change route or checkpoint instead of blind requeue.
</smarty-mergify-policy>
