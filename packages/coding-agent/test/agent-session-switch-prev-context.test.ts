...
describe("AgentSession.switchSession previous-context build", () => {
	...
	it("does not wait for retained async formatting before publishing a new target", async () => {
		...
		vi.spyOn(sessionManager, "saveArtifact").mockImplementation(async () => {
	...
	it("rejects reload when the session-before-switch hook cancels", async () => {
		...
		vi.spyOn(extensionRunner, "hasHandlers").mockImplementation(
			eventType => eventType === "session_before_switch" || hasHandlers(eventType),
		);
		const originalEmit = extensionRunner.emit.bind(extensionRunner);
		const emit = vi.spyOn(extensionRunner, "emit").mockImplementation((async event => {
			if (event.type === "session_before_switch") return { cancel: true };
