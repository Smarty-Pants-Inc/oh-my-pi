...
describe("AgentSession aside delivery", () => {
	...
	it("an idle session receiving an aside starts a turn", async () => {
		...
		expect(startedTurn).toEqual({ status: "accepted", delivery: "started_turn" });
	...
	it("sendCustomMessage(deliverAs: 'aside') folds into context instead of starting a turn when a user interrupt outlasted normalization", async () => {
		...
		expect(dispatched).toEqual({ status: "accepted", delivery: "plain_append" });
	...
	it("reports unavailable when an idle aside never dispatches", async () => {
		...
		expect(await dispatchedPromise).toEqual({ status: "unavailable", reason: "prompt_preflight_cancelled" });
		await session.waitForIdle();
		expect(contexts).toHaveLength(0);
	});
	...
	it("validates the session generation before an idle sendCustomMessage aside dispatch", async () => {
		...
		try {
			...
			expect(await dispatched).toEqual({ status: "unavailable", reason: "session_transition" });
