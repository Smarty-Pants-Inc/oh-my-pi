...
| YOLO workspace commands | The classifier first denied routine workspace checks, then a broad repair trusted arbitrary Bun scripts and process-spawning `rg`/Git options. | Built-in `bun test` has a bounded flag/path grammar; `bun run` requires an exact audited manifest script body; `rg` rejects pre-process/config injection; Git diff/log/show require no-ext-diff/no-textconv and reject helper/config injection; every routine command requires an in-workspace cwd. Unknown, network, destructive, and out-of-root commands remain capability-gated. | Bash session-capability positives and hostile publish/deploy/reset, `rg --pre`, Git helper/config, environment, and cwd-escape tests. |
