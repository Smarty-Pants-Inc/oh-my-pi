...
| YOLO workspace commands | The classifier first denied routine workspace checks, then a broad repair trusted arbitrary Bun scripts and process-spawning `rg`/Git options. | Built-in `bun test` has a bounded flag/path grammar; `bun run` requires an exact audited manifest script body; `rg` rejects pre-process/config injection; Git diff/log/show require no-ext-diff/no-textconv and reject helper/config injection. Routine commands require the current authoritative session root (including `/move`) or a workspace-owned cwd. Unknown, network, destructive, and other out-of-root commands remain capability-gated. | Bash session-capability positives; real `/move`; hostile publish/deploy/reset, `rg --pre`, Git helper/config, environment, and explicit cwd-escape tests. |
