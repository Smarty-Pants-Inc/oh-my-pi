# OMP 17.3.8 reconstruction patch ledger

This ledger records the deterministic reconstruction of the Smarty fork on the
frozen upstream target. The candidate starts at the exact upstream commit and
applies the net fork tree delta from the shared merge base to the pinned Smarty
fork. It excludes upstream-sync merges, does not adopt an unreviewed donor, and
records each substantive fork behavior once. The final candidate commit and
tree are reported outside this tracked file to avoid a self-referential commit
identity.
...
| Upstream fetch | `2026-08-20T00:07:26-04:00`, one canonical fetch |
| Upstream commit | `7e54061cbb1181dbc8dd7f0b37a1f12435a39e05` |
| Upstream tree | `b9d4d17c4aa51e674cfbbc39fd3eb0aebfc2e3e8` |
| Upstream version | `17.3.8` |
| Shared merge base | `8500092296621a6826b7136e840f8a59ea338958` |
| Shared merge-base tree | `7dca90c977b6f7b265fbcc7d1f8628f8b592b045` |
| Pinned Smarty fork | `6ca22b01fd3dc24e4b3b4d767b7f68550b9f8b0f` |
| Pinned Smarty tree | `d6a602a140d46371b986038ad20cb4bac5fa5543` |

The reconstruction branch starts directly at the upstream commit. It contains
no upstream-sync merge and no blind replay of fork history.

## Legacy Smarty fork patches

The source inventory is the reverse, no-merge history of
`8500092296621a6826b7136e840f8a59ea338958..6ca22b01fd3dc24e4b3b4d767b7f68550b9f8b0f`.
The commit identities below name causal source patches in the pinned fork; this
candidate rematerializes their accepted net behavior without importing that
history as ancestry.
...
| Fork-only `.gitignore` and package changelog edits | Split: obsolete metadata dropped; current user-visible notes retained | `.gitignore` and `packages/agent/CHANGELOG.md` remain byte-for-byte at upstream blobs `c4cea6ff0e9f3dc3e48b42bdd8856d2fb9c87160` and `f47be5dbb6f6a4fc505a5cfd2b87ed8a9a37f2c2`. `packages/coding-agent/CHANGELOG.md` retains exactly three `[Unreleased]` notes required by package changelog policy for the protected-prompt and scoped-peer-wake behaviors mapped below; it has no unrelated fork metadata. |
| Unmapped compatibility-only bytes | `DROP_OBSOLETE` | Scope admission found no directly changed implementation anchor for `packages/catalog/test/variant-collapse.test.ts`, `packages/coding-agent/test/profile-alias.test.ts`, `packages/coding-agent/test/update-cli.test.ts`, or `packages/mnemopi/test/native-vector-parity.test.ts`; the lazy worker/smoke import delta in `packages/coding-agent/src/cli.ts` also does not implement a Phase 1 requirement. All five paths are restored byte-for-byte to frozen upstream `7e54061cbb1181dbc8dd7f0b37a1f12435a39e05`. The eval agent-bridge runtime-profile assertions remain as the runnable dependency of the mapped structured-subagent profile seam. |
...
| `560e4b4b5` queue-safe CI and `.mergify.yml` | Retained and adapted | Current 17.3.8 workflow keeps queue refs and the always-run `smarty_ci` aggregate; serial one-at-a-time Mergify policy is in `b861df22d`. |

## Context and autonomy mission patches

These patches form a strict dependency chain. Each is retained against the
17.3.8 architecture and repaired at the shared runtime seam.
...
## Post-17.3.7 fork behavior groups

These patches landed after the prior reconstruction ledger was frozen. They are
part of the pinned fork delta and therefore require explicit disposition here.

| Source patches | Disposition | Reconstruction and proof boundary |
| --- | --- | --- |
| `252f69a0a` through `a022a9608` protected-prompt policy chain | Retained and reconciled | Rejected or malformed protected prompt hooks remain contained; policy drift is advisory and startup remains usable; unapproved protected builders stay disabled. Focused prompt-policy, hook, manifest, and startup tests are required before freezing the candidate. |
| `7c14e6ef0` owner goal lifecycle controls | Retained | Only the owner session may perform goal lifecycle transitions; subagent completion does not mutate parent todo or goal state. Goal, task, and interactive lifecycle tests are required. |
| `8a558f589`, `6598abdcd`, `f4bea4950`, `d2de2d015`, `ab9047f0e` scoped peer wakes | Retained and reconciled | Peer wake acceptance remains request-scoped, atomic, and fenced across handoff; the acceptance fence releases only after ownership transfers. Agent-loop, automatic-turn, concurrent-session, and handoff tests are required. |
| `bb8a35e27`, `795e60506`, `ac81d43d1`, `990cd2bfd` prompt lifecycle fencing | Retained and reconciled with upstream speculative maintenance | Prompts cannot enter during a handoff or cross queued-acceptance ownership; speculative handoff generation remains side-effect free until the fresh-session transaction commits. Handoff, maintenance, compaction, and lifecycle tests are required. |
| `387def255` provider response gates | Retained | Ollama and Devin response gates remain awaited at the final provider boundary. Provider payload and final-evidence tests are required. |
| `0683dc24b`, `20bbd38d6`, `a2df1c552`, `6ca22b01f` managed Herdr renderer bridge | Retained and reconciled | Managed renderer discovery remains nonfatal, session identity is revalidated after discovery, and bridge ownership survives handoff. Collaboration renderer, bridge, and install-smoke tests are required. |
...
```sh
git log --reverse --no-merges --format='%H %s' 8500092296621a6826b7136e840f8a59ea338958..6ca22b01fd3dc24e4b3b4d767b7f68550b9f8b0f
git diff --name-only 7e54061cbb1181dbc8dd7f0b37a1f12435a39e05..HEAD
(cd packages/coding-agent && bun run gen:prompt-manifest:check)
bun packages/coding-agent/src/cli.ts context manifest --json
bun packages/coding-agent/src/cli.ts context diff --base 7e54061cbb1181dbc8dd7f0b37a1f12435a39e05 --target HEAD --json
bun packages/coding-agent/src/cli.ts context protected-delta --repository . --base 7e54061cbb1181dbc8dd7f0b37a1f12435a39e05 --target HEAD --json
```
