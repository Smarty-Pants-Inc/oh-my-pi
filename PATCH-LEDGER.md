...
| Interactive policy drift | Startup blocked all OMP use on approval drift, while the capability bridge consumed permanent top-level tool context. | Interactive startup emits `PROMPT_POLICY_REVIEW_REQUIRED` and continues so the Stack guard can retain a visible warning; offline checks remain strict. `capability_grant` stays provenance-bound but moves to on-demand discovery. | Approved-policy warning and built-in load-mode tests. |
