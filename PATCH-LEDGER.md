...
| YOLO workspace commands | The classifier first denied routine workspace checks, then broad repairs trusted arbitrary Bun scripts, wrapper argv that only mentioned Git/GitHub, stale creation-time roots after `/move`, and process-spawning `rg`/Git options. | Built-in `bun test` has a bounded flag grammar and realpath-contained test paths; `bun run` requires an exact audited manifest script body and no unaudited lifecycle hook; Git/GitHub effect grants require the exact command-position executable; `rg` rejects pre-process/config injection; Git diff/log/show require no-ext-diff/no-textconv and reject helper/config injection. Routine reads and writes use the current authoritative session root after `/move` without rewriting creation-time grant provenance. Unknown, network, destructive, wrapper, lifecycle-hook, and out-of-root commands remain capability-gated. | Bash session-capability positives; real `/move` read/write plus stale-root denial; hostile lifecycle hook, symlink/Windows path, wrapper, publish/deploy/reset, `rg --pre`, Git helper/config, environment, and explicit cwd-escape tests. |
