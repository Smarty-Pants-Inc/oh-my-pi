...
| YOLO workspace commands | The classifier first denied routine workspace checks, then broad repairs trusted arbitrary Bun scripts, value-skipping boolean flags, wrapper argv that only mentioned Git/GitHub, executable-affecting environment overrides, stale creation-time roots after `/move`, and process-spawning `rg`/Git options. Structured write/edit tools still used the stale root. | Built-in `bun test` has an exact bounded flag grammar, inline numeric bail/parallel counts, and realpath-contained test paths; `bun run` requires an exact audited manifest script body and no unaudited lifecycle hook; Git/GitHub effect grants require the exact command-position executable with no executable/config override; `rg` rejects pre-process/config injection; Git diff/log/show require no-ext-diff/no-textconv and reject helper/config injection. One capability resolver authorizes Bash, write, edit, and ast_edit against the current session root after `/move` without rewriting creation-time grant provenance. Unknown, network, destructive, wrapper, lifecycle-hook, and out-of-root commands remain capability-gated. | Bash session-capability positives; real `/move` Bash/write/edit/ast_edit plus stale-root denial; hostile boolean-flag traversal, lifecycle hook, symlink/Windows path, wrapper, PATH/shell/loader/Git/GitHub overrides, publish/deploy/reset, `rg --pre`, Git helper/config, and explicit cwd-escape tests. |
