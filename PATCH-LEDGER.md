...
| --- | --- |
| `create` | Create a PR from an explicit already-published `--head`; title, body, base, draft, assignee, label, milestone, project, reviewer, and template fields are PR metadata. |
| `edit` | Change PR metadata, base selection, assignees, labels, projects, reviewers, milestone, title, or body. |
| `close` / `reopen` | Change PR open state without a comment or branch deletion. |
| `ready` | Change draft/readiness state. |
| `review` | Approve or request changes; comment-only review mode is excluded. |
| `lock` / `unlock` | Change the PR conversation lock state. |

No admitted subcommand mutates a Git ref, updates the PR head, creates an issue
comment, queues or merges the PR, deletes a branch, or launches an editor or browser.
