...
| --- | --- |
| `create` | Create a PR from an explicit already-published `--head` into explicit `base=main`; title, body, draft, assignee, label, milestone, project, reviewer, and template fields are PR metadata. |
| `edit` | Change PR metadata, assignees, labels, projects, reviewers, milestone, title, or body; base selection is excluded. |
