...
| Unmapped compatibility-only bytes | `DROP_OBSOLETE` | Scope admission found no directly changed implementation anchor for `packages/catalog/test/variant-collapse.test.ts`, `packages/coding-agent/test/eval/agent-bridge-policy.test.ts`, `packages/coding-agent/test/profile-alias.test.ts`, `packages/coding-agent/test/update-cli.test.ts`, or `packages/mnemopi/test/native-vector-parity.test.ts`; the lazy worker/smoke import delta in `packages/coding-agent/src/cli.ts` also does not implement a Phase 1 requirement. All six paths are restored byte-for-byte to frozen upstream `37eee71978951fccf66b21f7e3e2b74596ac9d74`. |
